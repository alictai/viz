class Treemap {
  ...
  int canvas_x1, canvas_y1;

  Treemap(Canvas node, int x_1, int y_1, int wid, int hgt) {
    root = node;
    active_node = node;
    rows = new Row[0];
    active_level = 0;
  ...
  void draw_treemap() {
      ...
      //draw active_node
      new_row(active_node, canvas_x1, canvas_y1, canvas_w, canvas_h, 0);
      
      //recursively draw all children
      squarify(active_node, canvas_x1, canvas_y1, canvas_w, canvas_h, 0);
      draw_rows();
  }
  ...
  void draw_rows() {
      for(int i = 0; i < rows.length; i++) {
          for(int k = 0; k < rows[i].values.length; k++) {
              ...
              if(rows[i].values[k].id != 10) {
                if(d.isMarked(rows[i].values[k].id)) {
                  ellipse(rows[i].values[k].x+(rows[i].values[k].wid/2), rows[i].values[k].y+(rows[i].values[k].hgt/2), 5, 5);
                }
              }
        }
      }
