class Pie_Chart {
  ...
  Pie_Chart(Data parsed, int c_x1, int c_y1, int c_x2, int c_y2) {
    phase = 0;
    data = parsed;
    find_ymax();
 ...
 void find_ymax() {
  ...
  void find_angles() {
    ...
    for (int k = 0; k < data.getLength(); k++) {
      float angle = float(360) * data.getVal(k) / total;
  ...
  void make_chart(color text_c, boolean shrink, boolean spread, boolean rotate, boolean collapse) {
      float lastAngle = 0;
      int center_x = (canvas_x1 + canvas_x2)/2;
      int center_y = (canvas_y1 + canvas_y2)/2;
      
      for (int i = 0; i < data.getLength(); i++) {
          float gray = map(i, 0, data.getLength(), 0, 255);
          fill(255);
         
          if(!shrink && !spread && !rotate && !collapse) {
              arc(center_x, center_y, diameter, diameter, lastAngle, lastAngle + radians(angles[i]), PIE);
              //draw_words(text_c, lastAngle, i);
          } else if (shrink && !spread && !rotate && !collapse) {
              arc(center_x, center_y, dum_dia, dum_dia, lastAngle, lastAngle + radians(angles[i]), PIE);
          } else if (shrink && !spread && rotate && !collapse) {  //special pie_to_rose mode
              arc(center_x, center_y, dum_dia, dum_dia, radians(dum_last_ang[i]), radians(dum_last_ang[i] + dum_angles[i]), PIE);
          } else if (shrink && spread && !rotate && !collapse) {
              arc(center_x, center_y, dum_dia, dum_dia, lastAngle, lastAngle + radians(angles[i]), PIE);
          } else if (shrink && spread && rotate && !collapse) {
              arc(center_x, center_y, dum_dia, dum_dia, dum_last_ang[i], dum_last_ang[i] + radians(angles[i]), PIE);
          } else {//(shrink && spread && rotate && collapse)
              arc(center_x, center_y, dum_dia, dum_dia, dum_last_ang[i], radians(dum_angles[i]), PIE);
          }
  ...
  void find_diameter() {
   ...
   int set_ptol_dummy() {
       ...
       dum_dia = diameter;
       dum_dia_target = 150;
       dum_angles = new float[data.getLength()];
       dum_last_ang = new float[data.getLength()];
       spread_loc = new float[data.getLength()];
       dum_height = new float[data.getLength()];
       float ang_trak = 0;
   ...
   int set_btop_dummy(float[] y_coords, float[] x_coords, float w) {
       ...
       dum_angles = new float[data.getLength()];
       dum_last_ang = new float[data.getLength()];
       dum_last_ang_corr = new float[data.getLength()];
       spread_loc = new float[data.getLength()];
       dum_height = new float[data.getLength()];
       float ang_trak = 0; 
